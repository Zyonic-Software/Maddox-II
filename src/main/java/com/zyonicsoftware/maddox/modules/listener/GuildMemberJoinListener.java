...
public class GuildMemberJoinListener extends ListenerAdapter {
  ...
  public GuildMemberJoinListener(final Maddox maddox) { this.maddox = maddox; }
  ...
  private void sendJoinMessage(final GuildMemberJoinEvent event) {
    if (this.maddox.getCacheManager().isJoinMessageEnabled(
            event.getGuild().getId())) {
      final String channelID =
          this.maddox.getCacheManager().getJoinMessageChannel(
              event.getGuild().getId());
      if (channelID != null) {
