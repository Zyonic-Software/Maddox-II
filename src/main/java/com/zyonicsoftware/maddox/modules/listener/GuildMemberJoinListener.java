...
public class GuildMemberJoinListener extends ListenerAdapter {
  ...
  public GuildMemberJoinListener(Maddox maddox) { this.maddox = maddox; }
  ...
  private void assignAutoRoles(GuildMemberJoinEvent event) {
    ...
    this.maddox.getAutomaticRoleManager()
        .getRolesForAutomaticAssigning(new MaddoxGuild(
            event.getGuild(),
            this.maddox.getMySQLHandler().getPrefix(event.getGuild().getId()),
            this.maddox.getMySQLHandler()))
        .forEach(role -> { member.addRole(role).queue(); });
  }

  private void sendJoinMessage(GuildMemberJoinEvent event) {
    if (this.maddox.getMySQLHandler().isJoinMessageEnabled(
            event.getGuild().getId())) {
      String channelID = this.maddox.getMySQLHandler().getJoinMessageChannel(
          event.getGuild().getId());
      if (channelID != null) {
        Objects.requireNonNull(event.getGuild().getTextChannelById(channelID))
            .sendMessage(
                this.maddox.getMySQLHandler()
                    .getJoinMessage(event.getGuild().getId())
