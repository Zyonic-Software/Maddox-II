...
public class GuildMemberLeaveListener extends ListenerAdapter {
  ...
  public GuildMemberLeaveListener(Maddox maddox) { this.maddox = maddox; }
  ...
  private void sendLeaveMessage(GuildMemberRemoveEvent event) {
    if (this.maddox.getMySQLHandler().isLeaveMessageEnabled(
            event.getGuild().getId())) {
      String channelID = this.maddox.getMySQLHandler().getLeaveMessageChannel(
          event.getGuild().getId());
      if (channelID != null) {
        Objects.requireNonNull(event.getGuild().getTextChannelById(channelID))
            .sendMessage(
                this.maddox.getMySQLHandler()
                    .getLeaveMessage(event.getGuild().getId())
                    .replace("<USER>",
