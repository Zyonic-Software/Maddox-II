...
public class GuildMemberLeaveListener extends ListenerAdapter {
  ...
  public GuildMemberLeaveListener(final Maddox maddox) { this.maddox = maddox; }
  ...
  private void sendLeaveMessage(final GuildMemberRemoveEvent event) {
    if (this.maddox.getCacheManager().isLeaveMessageEnabled(
            event.getGuild().getId())) {
      final String channelID =
          this.maddox.getCacheManager().getLeaveMessageChannel(
              event.getGuild().getId());
      if (channelID != null) {
