...
public class HelpCommand extends Command {
    ...
    public HelpCommand(Maddox maddox) {
        ...
        this.setDescription("Shows Help-Information");
    ...
    protected void execute(CommandEvent event) {
        if (event.getArguments().isEmpty()) {
            event.reply(maddox.getHelpBuilder().assembleHelp(event.getMaddoxMember(), event.getPrefix()));
            ...
            event.reply(this.maddox.getHelpBuilder().generateCommandHelp(this.maddox.getCommandHandler().getCommands().get(event.getArguments().get(0).toLowerCase()), event.getPrefix()));
        }
    }
}
