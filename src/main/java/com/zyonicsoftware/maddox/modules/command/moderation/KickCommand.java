  ...
  protected void execute(CommandEvent event, MaddoxMember sender,
    ...
    if (sender.hasPermission(Permission.KICK_MEMBERS)) {
      if (!event.getMentions().isEmpty()) {
        if (event.getArguments().size() > 1) {
          ...
          event.reply(
              new EmbedBuilder()
                  .addField(
                      "Kick",
                      LanguageAPI.getValue("Kick-With-Reason")
                          .replace("<USER>",
                                   event.getMentions().get(0).getUsername())
                          .replace("<REASON>",
                                   event.getArgumentsAsString().replace(
                                       " " +
                                           event.getMentions()
                                               .get(0)
                                               .getAsMention() +
          ...
          event.reply(
              new EmbedBuilder()
                  .addField(
                      "Kick",
                      LanguageAPI.getValue("Kick-Without-Reason")
                          .replace("<USER>",
                                   event.getMentions().get(0).getUsername()),
                      false)
                  .setColor(this.maddox.getDefaultColor())
        ...
        event.reply(
            LanguageAPI.getValue("Kick-NoPersonDefined", server.getLanguage())
                .replace("<PREFIX>", event.getPrefix()));
      }
    }
  }
}
