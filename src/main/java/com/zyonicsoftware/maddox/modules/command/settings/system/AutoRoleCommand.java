    ...
    if (event.getArguments().size() > 1) {
      if (event.getArguments().get(0).equalsIgnoreCase("add")) {
        if (!event.getRoleMentions().isEmpty()) {
          if (this.maddox.getAutomaticRoleManager()
                  .addRolesToAutomaticAssigning(
                      (ArrayList<Role>)event.getRoleMentions(), server)) {
            event.reply(
                LanguageAPI
                    .getValue("AutoRole-Response-1", server.getLanguage())
                    .replace(
                        "<ROLE>",
                        event.getRoleMentions().get(0).getName())); // Success
          } else {
            event.reply(LanguageAPI.getValue(
                "NoChange", server.getLanguage())); // Didnt Change
          }
        }
      } else if (event.getArguments().get(0).equalsIgnoreCase("remove")) {
        if (!event.getRoleMentions().isEmpty()) {
          if (this.maddox.getAutomaticRoleManager()
                  .removeRolesFromAutomaticAssigning(
                      (ArrayList<Role>)event.getRoleMentions(), server)) {
            event.reply(
                LanguageAPI
                    .getValue("AutoRole-Response-2", server.getLanguage())
                    .replace(
                        "<ROLE>",
                        event.getRoleMentions().get(0).getName())); // Success
          } else {
      ...
      event.reply(
          LanguageAPI.getValue("AutoRole-Response-3", server.getLanguage()) +
          LanguageAPI.getValue("AutoRole-Syntax", server.getLanguage())
