...
public class PreStartupLoader {
  ...
  public PreStartupLoader(final Maddox maddox) { this.maddox = maddox; }

  public void loadConfigFile(final BaseValueConfig config,
  ...
  private void initMainConfig(final BaseValueConfig config) {
    try {
      ...
      if (!configFile.createNewFile()) {
        ...
        yamlConfiguration.set("botAdministrator",
                              "309007975060602882,498077062775570432");
  ...
  private void initMysqlConfig(final MySQLConfig mySQLConfig) {
    try {
      ...
      if (!mysqlFile.createNewFile()) {
        ...
        final YamlConfiguration yamlConfiguration =
            YamlConfiguration.loadConfiguration(mysqlFile);
        ...
        yamlConfiguration.set(
            "password", "maddox_is_cool_please_use_a_safe_password_i_beg_you");

        yamlConfiguration.save(mysqlFile);

        System.out.println(
            "Generated mySQL config, please enter your Information and restart");
      }
    } catch (final Exception e) {
      e.printStackTrace();
    }
