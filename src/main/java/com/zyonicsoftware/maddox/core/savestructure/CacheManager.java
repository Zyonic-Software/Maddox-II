...
public class CacheManager {
  ...
  private final HashMap<String, HashMap<Datatype, String>> guildMap =
      new HashMap<>();
  private final HashMap<String, HashMap<Toggletype, Boolean>> guildToggleMap =
      new HashMap<>();

  public CacheManager(final Maddox maddox) { this.maddox = maddox; }

  private void registerServerToCache(final String guildID) {
    final HashMap<Datatype, String> guildData = new HashMap<>();
    guildData.put(Datatype.PREFIX,
                  this.maddox.getMySQLHandler().getPrefix(guildID));
    guildData.put(Datatype.LANGUAGE,
                  this.maddox.getMySQLHandler().getServerLanguage(guildID));
    guildData.put(
        Datatype.AUTOROLES,
        this.maddox.getMySQLHandler().getRolesForAutomaticAssigning(guildID));
    guildData.put(Datatype.JOINCHANNEL,
                  this.maddox.getMySQLHandler().getJoinMessageChannel(guildID));
    guildData.put(Datatype.JOINMESSAGE,
                  this.maddox.getMySQLHandler().getJoinMessage(guildID));
    guildData.put(
        Datatype.LEAVECHANNEL,
        this.maddox.getMySQLHandler().getLeaveMessageChannel(guildID));
    guildData.put(Datatype.LEAVEMESSAGE,
                  this.maddox.getMySQLHandler().getLeaveMessage(guildID));
    guildData.put(Datatype.ENABLED_COMMANDS,
                  this.maddox.getMySQLHandler().getEnabledCommands(guildID));
    ...
    try {
      guildToggleData.put(
          Toggletype.JOINMESSAGE,
          this.maddox.getMySQLHandler().isJoinMessageEnabled(guildID));
    ...
    try {
      guildToggleData.put(
          Toggletype.LEAVEMESSAGE,
          this.maddox.getMySQLHandler().isLeaveMessageEnabled(guildID));
    } catch (final Exception ignored) {
  ...
  public void
  setRolesForAutomaticAssigning(final String rolesforautomaticassigning,
                                final String guildID) {
    if (!this.guildMap.containsKey(guildID)) {
      this.registerServerToCache(guildID);
    }
    this.guildMap.get(guildID).put(Datatype.AUTOROLES,
                                   rolesforautomaticassigning);
    this.maddox.getMySQLHandler().setRolesForAutomaticAssigning(
        rolesforautomaticassigning, guildID);
  }
  ...
  public void setJoinMessageChannel(final String joinMessageChannelID,
                                    final String guildID) {
    if (!this.guildMap.containsKey(guildID)) {
      this.registerServerToCache(guildID);
    }
    this.guildMap.get(guildID).put(Datatype.JOINCHANNEL, joinMessageChannelID);
    this.maddox.getMySQLHandler().setJoinMessageChannel(joinMessageChannelID,
                                                        guildID);
  }
  ...
  public void setLeaveMessageChannel(final String leaveMessageChannelID,
                                     final String guildID) {
    if (!this.guildMap.containsKey(guildID)) {
      this.registerServerToCache(guildID);
    }
    this.guildMap.get(guildID).put(Datatype.LEAVECHANNEL,
                                   leaveMessageChannelID);
    this.maddox.getMySQLHandler().setLeaveMessageChannel(leaveMessageChannelID,
                                                         guildID);
  }
  ...
  public void setEnabledCommands(final String enabledCommandsInString,
                                 final String guildID) {
    if (!this.guildMap.containsKey(guildID)) {
      this.registerServerToCache(guildID);
    }
    this.guildMap.get(guildID).put(Datatype.LEAVECHANNEL,
                                   enabledCommandsInString);
    this.maddox.getMySQLHandler().setEnabledCommands(enabledCommandsInString,
                                                     guildID);
  }
  ...
  public void setLeaveMessageEnabled(final boolean state,
                                     final String guildID) {
    if (!this.guildToggleMap.containsKey(guildID)) {
      this.registerServerToCache(guildID);
    }
