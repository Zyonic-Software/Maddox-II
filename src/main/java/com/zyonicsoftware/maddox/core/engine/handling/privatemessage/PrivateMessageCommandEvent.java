...
public class PrivateMessageCommandEvent {
  ...
  public PrivateMessageCommandEvent(final PrivateMessageCommand command,
  ...
  public PrivateMessageCommand getCommand() { return this.command; }

  public PrivateMessageReceivedEvent getEvent() { return this.event; }

  public User getAuthor() { return this.event.getAuthor(); }

  public PrivateChannel getChannel() { return this.event.getChannel(); }

  public Message getMessage() { return this.event.getMessage(); }

  public JDA getJDA() { return this.event.getJDA(); }
  ...
  public ArrayList<String> getArguments() {
    String arguments;
    arguments = this.event.getMessage().getContentRaw().substring(
        this.command.getName().length() +
        this.maddox.getDefaultPrefix().length());
    if (this.event.getMessage().getContentRaw().startsWith(
            this.maddox.getDefaultPrefix() + " ")) {
      arguments = this.event.getMessage().getContentRaw().substring(
          this.command.getName().length() +
          this.maddox.getDefaultPrefix().length() + 1);
    }
