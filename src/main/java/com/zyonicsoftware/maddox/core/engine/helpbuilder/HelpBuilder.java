...
public class HelpBuilder {
  ...
  public HelpBuilder(final Maddox maddox) { this.maddox = maddox; }

  public MessageEmbed assembleHelp(final MaddoxMember maddoxMember,
                                   final String prefix, final String language) {
    final HashMap<String, Command> commands =
        this.maddox.getCommandHandler().getCommands();
    final HashMap<String, Command> specefiedPrefixCommands =
        this.maddox.getCommandHandler().getSpecificPrefixCommands();
    final HashMap<String, ArrayList<Command>> sortedCommands = new HashMap<>();
    final HashMap<String, ArrayList<Command>> sortedSpecefiedPrefixCommands =
        new HashMap<>();

    final EmbedBuilder embedBuilder = new EmbedBuilder();

    embedBuilder.setTitle("**" + this.maddox.getName() + " Help**")
        .setThumbnail(
            maddoxMember.getCurrentJDA().getSelfUser().getAvatarUrl());
    embedBuilder.setColor(this.maddox.getDefaultColor());

    // Sorting commands into specefied Categorys
    commands.forEach((name, command) -> {
      if (command.ShowInHelp()) {
        if (command.getCommandHelpViewPermission() <=
            maddoxMember.getCommandHelpViewPermissionValue()) {
          if (command.isGetValuesFromLanguageYaml()) { // if Gets Values from
                                                       // Language-Yaml
            if (sortedCommands.containsKey(
                    LanguageAPI.getValue(command.getCategory(), language))) {
              sortedCommands
                  .get(LanguageAPI.getValue(command.getCategory(), language))
                  .add(command);
            } else {
              sortedCommands.put(
                  LanguageAPI.getValue(command.getCategory(), language),
    ...
    specefiedPrefixCommands.forEach((name, command) -> {
      if (command.getCommandHelpViewPermission() <=
        ...
        if (command.isGetValuesFromLanguageYaml()) { // if Gets Values from
          ...
          if (sortedCommands.containsKey(
                  LanguageAPI.getValue(command.getCategory(), language))) {
            sortedSpecefiedPrefixCommands
                .get(LanguageAPI.getValue(command.getCategory(), language))
                .add(command);
          } else {
            sortedSpecefiedPrefixCommands.put(
                LanguageAPI.getValue(command.getCategory(), language),
                new ArrayList<>());
            sortedSpecefiedPrefixCommands
                .get(LanguageAPI.getValue(command.getCategory(), language))
          ...
          if (command.ShowInHelp()) {
            if (sortedSpecefiedPrefixCommands.containsKey(
                    command.getCategory())) {
              sortedSpecefiedPrefixCommands.get(command.getCategory())
                  .add(command);
            } else {
              sortedSpecefiedPrefixCommands.put(command.getCategory(),
                                                new ArrayList<>());
              sortedSpecefiedPrefixCommands.get(command.getCategory())
                  .add(command);
            }
    ...
    if (sortedSpecefiedPrefixCommands.get("none") != null) {
      ...
      embedBuilder.addField(
          "**Other • " + sortedSpecefiedPrefixCommands.get("none").size() +
              "**",
          commandsWithoutCategoryShow.toString(), false);
    }

    return embedBuilder.build();
  }

  public MessageEmbed generateCommandHelp(final Command command,
                                          final String prefix,
                                          final MaddoxMember maddoxMember,
      ...
      if (command.isGetValuesFromLanguageYaml()) {
        if (command.getDescription() != null) {
          return new EmbedBuilder()
              ...
              .addField(
                  LanguageAPI
                      .getValue(command.getSyntax(), server.getLanguage())
                      .replace("<PREFIX>", prefix),
                  LanguageAPI
                      .getValue(command.getDescription(), server.getLanguage())
                      .replace("<PREFIX>", prefix),
                  false)
              .build();
        } else {
          return new EmbedBuilder()
              ...
              .addField(
                  ...
                  "Please either Disable the .showExtendedHelp option in your CommandClass or add a Description & a Syntax-Description",
                  false)
              .setThumbnail(
                  maddoxMember.getCurrentJDA().getSelfUser().getAvatarUrl())
              .build();
        ...
        if (command.getDescription() != null) {
          return new EmbedBuilder()
              .setTitle("**" + prefix + command.getName() + "**")
              .setColor(this.maddox.getDefaultColor())
              .addField(command.getSyntax().replace("<PREFIX>", prefix),
                        command.getDescription().replace("<PREFIX>", prefix),
                        false)
              .setThumbnail(
                  maddoxMember.getCurrentJDA().getSelfUser().getAvatarUrl())
              .build();
