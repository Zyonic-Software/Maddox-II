...
public class CommandEvent {
  ...
  public CommandEvent(Command command, GuildMessageReceivedEvent receivedEvent,
                      String prefix) {
  ...
  public CommandEvent(Command command, GuildMessageUpdateEvent updateEvent,
                      String prefix) {
    this.command = command;
  ...
  public String getArgumentsAsString() {
    if (!this.getArguments().isEmpty()) {
      if (receivedEvent != null) {
        return receivedEvent.getMessage().getContentRaw().substring(
            command.getName().length() + prefix.length() + 1);
      } else {
        return updateEvent.getMessage().getContentRaw().substring(
            command.getName().length() + prefix.length() + 1);
      }
    } else {
      return "";
  ...
  public List<String> getArguments() {
    String arguments;
    if (receivedEvent != null) {
      arguments = receivedEvent.getMessage().getContentRaw().substring(
          command.getName().length() + prefix.length());
      if (receivedEvent.getMessage().getContentRaw().startsWith(prefix + " ")) {
        arguments = receivedEvent.getMessage().getContentRaw().substring(
            command.getName().length() + prefix.length() + 1);
      }
    } else {
      arguments = updateEvent.getMessage().getContentRaw().substring(
          command.getName().length() + prefix.length());
  ...
  public String getExecutorAvatar(int imageDimension) {
    if (receivedEvent != null) {
      return "https://cdn.discordapp.com/avatars/" +
          receivedEvent.getMember().getId() + "/" +
          receivedEvent.getMember().getUser().getAvatarId() +
          ".png?size=" + imageDimension;
    } else {
      return "https://cdn.discordapp.com/avatars/" +
          updateEvent.getMember().getId() + "/" +
          updateEvent.getMember().getUser().getAvatarId() +
          ".png?size=" + imageDimension;
    }
  }
  ...
  public MaddoxGuild getServer() { return server; }
}
