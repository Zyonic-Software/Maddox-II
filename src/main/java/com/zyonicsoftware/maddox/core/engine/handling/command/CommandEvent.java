...
public class CommandEvent {
  ...
  // Constructors Differenciating between Events, Command Event also gets
  // Triggered on MessageEdit, if enabled in Command
  public CommandEvent(final Command command,
                      final GuildMessageReceivedEvent receivedEvent,
                      final String prefix) {
  ...
  public CommandEvent(final Command command,
                      final GuildMessageUpdateEvent updateEvent,
                      final String prefix) {
    this.command = command;
  ...
  public Category getCategory() { return this.getChannel().getParent(); }
  ...
  public String getArgumentsAsString() {
    if (!this.getArguments().isEmpty()) {
      if (this.receivedEvent != null) {
        return this.receivedEvent.getMessage().getContentRaw().substring(
            this.command.getName().length() + this.prefix.length() + 1);
      } else {
        return this.updateEvent.getMessage().getContentRaw().substring(
            this.command.getName().length() + this.prefix.length() + 1);
      }
    } else {
      return "";
  ...
  public List<String> getArguments() {
    String arguments;
    if (this.receivedEvent != null) {
      arguments = this.receivedEvent.getMessage().getContentRaw().substring(
          this.command.getName().length() + this.prefix.length());
      if (this.receivedEvent.getMessage().getContentRaw().startsWith(
              this.prefix + " ")) {
        arguments = this.receivedEvent.getMessage().getContentRaw().substring(
            this.command.getName().length() + this.prefix.length() + 1);
      }
    } else {
      arguments = this.updateEvent.getMessage().getContentRaw().substring(
          this.command.getName().length() + this.prefix.length());
    ...
    if (arguments.length() > 0) {
      ...
      final List<String> argslist =
          new ArrayList<>(Arrays.asList(arguments.split(" ")));
  ...
  public String getExecutorAvatar(final int imageDimension) {
    if (this.receivedEvent != null) {
      return "https://cdn.discordapp.com/avatars/" +
          this.receivedEvent.getMember().getId() + "/" +
          this.receivedEvent.getMember().getUser().getAvatarId() +
          ".png?size=" + imageDimension;
    } else {
      return "https://cdn.discordapp.com/avatars/" +
          this.updateEvent.getMember().getId() + "/" +
          this.updateEvent.getMember().getUser().getAvatarId() +
          ".png?size=" + imageDimension;
    }
  }
  ...
  public List<MaddoxMember> getMentions() {
    ...
    memberIDs.forEach(userID -> {
      try {
        final Member member =
            this.getGuild().retrieveMemberById(userID).complete(true);
        if (member != null) {
  ...
  public MaddoxGuild getServer() { return this.server; }
}
